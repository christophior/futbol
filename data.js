...
const updateScheduleData = (followedMatch) => {
  ...
  getScheduleData((scheduleData) => {
    ...
    updateScheduleView(
      leagues[settings.get("selectedLeague", 0)],
...
const getSchedulesUrl = ({ season, competition }) =>
  `https://api.fifa.com/api/v1/calendar/matches?idseason=${season}&idcompetition=${competition}&language=en-GB&count=1000`;

const getScheduleData = (next) => {
  console.log(`Getting schedule data`);
  const scheduleUrl = getSchedulesUrl(
      leagues[settings.get("selectedLeague", 0)]
    ),
  ...
  axios
    ...
    .then(
      checkFailed(([scheduleResponse, liveResponse]) => {
        let data =
            (scheduleResponse.data && scheduleResponse.data.Results) || [],
    ...
    .catch((err) => {
      ...
      if (err[0].status === 200 && err[1].status === 200) {
        ...
        scheduleData = processScheduleData(
          substituteLiveData(scheduleResponse, liveResponse)
        );
      } else if (err[0].status === 200) {
        let scheduleResponse = (err[0].data && err[0].data.Results) || [];
        scheduleData = processScheduleData(
          substituteLiveData(scheduleResponse, [])
        );
      }
...
const substituteLiveData = (data, liveData) => {
  data.forEach((match, index) => {
    ...
    let foundLiveMatch = isLiveMatch
      ? liveData.find((m) => m.IdMatch == match.IdMatch)
      : null;
...
const formatImage = (url) =>
  url ? url.replace("{format}", "sq").replace("{size}", "2") : "";
...
const processScheduleData = (matchList) => {
  ...
  matchList.forEach((match) => {
    let {
      ...
      isCancelledMatch = matchStatus === 7,
      isFutureMatch =
        homeScore === null ||
        awayScore === null ||
        (!isPastMatch && !isLiveMatch && !isCancelledMatch);
  ...
  matches.future = matches.future.filter(
    ({ homeTeam, awayTeam }) => homeTeam !== "TBD" && awayTeam !== "TBD"
  );
